...
if (fs.existsSync(dockerTemplatePath)) {
  ...
  fs.writeFileSync(
    path.resolve(publishDir, 'Dockerfile'),
    fileContent.toString().replace(/@distName/g, distName),
  );
} else {
...
if (fs.existsSync(startupShellPath)) {
  ...
  fs.writeFileSync(
    path.resolve(publishDir, 'startup-docker-container.sh'),
    fileContent.toString().replace(/@projName/g, pkg.name),
  );
} else {
  console.warn('[Warn]未能生成Dockerfile');
}
